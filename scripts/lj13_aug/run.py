...
from en_flows.dw4_experiment.dataset import get_data_dw4, get_data_lj13, remove_mean
import sake
...
def run(n_data):
    ...
    data_train, batch_iter_train = get_data_lj13(n_data, 'train', 100)
    data_val, batch_iter_val = get_data_lj13(100, 'val', 100)
    data_test, batch_iter_test = get_data_lj13(100, 'test', 100)
    ...
    def epoch(state, key):
        idxs = jax.random.permutation(jax.random.PRNGKey(state.step), len(data_train))
